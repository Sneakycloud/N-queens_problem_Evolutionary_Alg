...
def n_queen_plot(itererations, board_sizes_n : list, boards_per_generation, children_per_generation, mutation_rate, max_generations, stall_limit, ignore_failed_attempts, pop_init_algorithm, main_algorithm, print_to_file, txt_file_name):
    ...
    #ploting graph
    plt.subplot(311)
    plt.plot(board_sizes_n[:-1],avg_time_taken_n)
    plt.axis((4,max_board_size, 0, math.ceil(max(avg_time_taken_n))))
    plt.ylabel("Avg time (s)")
    
    plt.subplot(312)
    plt.plot(board_sizes_n[:-1],avg_generations_taken_n)
    plt.axis((4,max_board_size, 0, math.ceil(max(avg_generations_taken_n))))
    plt.ylabel("Avg generations taken")

    plt.subplot(313)
    plt.plot(board_sizes_n,probaility_of_success)
    plt.axis((4,max_board_size, 0, 1))
...
plt.subplot(311)
plt.plot([x for x in range(4,14)],[0.00234375, 0.0025, 0.003125, 0.23, 4.2509375, 20.06171875, 26.19046875, 29.528125, 34.77828125, 38.84765625])
plt.axis((0,14, 0, 40))
plt.ylabel("Avg time (s)")

plt.subplot(312)
plt.plot([x for x in range(4,14)],[0, 0, 0, 0, 42.95, 736.11, 3180.57, 3782.71, 3765.97, 3960.0])
plt.axis((0,14, 0, 4000))
plt.ylabel("Avg generations taken")

plt.subplot(313)
plt.plot([x for x in range(4,15)],[1, 1, 1, 1, 1, 93/100, 26/100, 9/100, 6/100, 1/100, 0])
plt.axis((0,14, 0, 1))
plt.xlabel("Size of n")
plt.ylabel("Success rate")
