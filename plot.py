...
def n_queen_plot(itererations, board_sizes_n : list, boards_per_generation, children_per_generation, mutation_rate, max_generations, stall_limit, ignore_failed_attempts, pop_init_algorithm, main_algorithm):
    ...
    for board_size_n in board_sizes_n:
        ...
        if(main_algorithm == 0):
            _, generations_taken, time_taken, successes = info_base_n_queen_solver(itererations, board_size_n, boards_per_generation, children_per_generation, mutation_rate, ignore_failed_attempts, max_generations)
        elif (main_algorithm == 1):
            _, generations_taken, time_taken, successes = info_n_queen_solver(itererations, board_size_n, boards_per_generation, mutation_rate, max_generations, stall_limit, ignore_failed_attempts, pop_init_algorithm)
