...
def avg_time_n_queens_solver(board_size_n, boards_per_generation,pop_init_alg, mutation_rate, max_generations, stall_limit, itererations, ignore_failed_attempts, ignore_0_gen, n_queen_solver):
    time_taken = []
...
def gen_size_tuner(board_size_n, boards_per_generation,pop_init_alg, mutation_rate, max_generations, stall_limit,stop_tolerence,itererations, ignore_failed_attempts, n_queen_solver):
    ...
    def parameter_avg_time_n_queens_solver(gen_size):
        return avg_time_n_queens_solver(board_size_n, gen_size,pop_init_alg, mutation_rate, max_generations, stall_limit, itererations, ignore_failed_attempts, False, n_queen_solver)
    ...
    while not is_successfull:
        ...
        for _ in range(itererations):
            solution_found = (n_queen_solver(low_search_value, boards_per_generation, mutation_rate, max_generations, stall_limit, pop_init_alg))
    ...
    while low_search_value != high_search_value:
        ...
        if high_search_value == low_search_value and high_search_value == max_search_value:
...
def mutation_rate_tuner(board_size_n, boards_per_generation,pop_init_alg, mutation_rate, max_generations, stall_limit, stop_tolerence,itererations, ignore_failed_attempts, n_queen_solver):
    calibrated_mutation_rate = mutation_rate // 2
    last_calibrated_mutation_rate = 101
    last_low_search_value = mutation_rate
    last_high_search_value = 0
    
    low_search_value = 0
    max_search_value = mutation_rate
    high_search_value = 100
    
    current_time_taken = 0
    half_time_taken = 0
    double_time_taken = 0
    
    current_time_taken = avg_time_n_queens_solver(board_size_n, boards_per_generation,pop_init_alg, calibrated_mutation_rate, max_generations, stall_limit, itererations, ignore_failed_attempts, n_queen_solver)
    
    while low_search_value != high_search_value:
        print(f"Mutation now testing: {(calibrated_mutation_rate // 2 + low_search_value) // 2}, {calibrated_mutation_rate}, {(calibrated_mutation_rate * 2 + high_search_value) // 2}")
        
        #tests if the last value was just tested. This is to avoid the function bouceing between two values
        if (abs(max(calibrated_mutation_rate, low_search_value, high_search_value) - min(calibrated_mutation_rate, low_search_value, high_search_value)) < 2 or abs(max(current_time_taken, half_time_taken, double_time_taken) - min (current_time_taken, half_time_taken, double_time_taken)) < stop_tolerence):
            break
        
        #Check half the value
        if(last_low_search_value != low_search_value):
            half_time_taken = avg_time_n_queens_solver(board_size_n, boards_per_generation,pop_init_alg, (calibrated_mutation_rate // 2 + low_search_value) // 2, max_generations, stall_limit, itererations, ignore_failed_attempts, n_queen_solver)
        
        #Check if we need to update the current value
        if(last_calibrated_mutation_rate != calibrated_mutation_rate):
            double_time_taken = avg_time_n_queens_solver(board_size_n, boards_per_generation,pop_init_alg, calibrated_mutation_rate, max_generations, stall_limit, itererations, ignore_failed_attempts, n_queen_solver)
        
        
        #Check double the value
        if(last_high_search_value != high_search_value):
            double_time_taken = avg_time_n_queens_solver(board_size_n, boards_per_generation,pop_init_alg, (calibrated_mutation_rate * 2 + high_search_value) // 2, max_generations, stall_limit, itererations, ignore_failed_attempts, n_queen_solver)
        
        last_low_search_value = low_search_value
        last_high_search_value = high_search_value
        last_calibrated_mutation_rate = calibrated_mutation_rate
        
        #Adjust based on if the half, current or double lead to the best result            
        if half_time_taken == min(current_time_taken, half_time_taken, double_time_taken):
            high_search_value = calibrated_mutation_rate
            double_time_taken = current_time_taken
            calibrated_mutation_rate = (calibrated_mutation_rate + low_search_value) // 2
            
            last_high_search_value = high_search_value
        elif double_time_taken == min(current_time_taken, half_time_taken, double_time_taken):
            low_search_value = calibrated_mutation_rate
            half_time_taken = current_time_taken
            calibrated_mutation_rate = (calibrated_mutation_rate + high_search_value) // 2
            
            last_low_search_value = low_search_value
        elif current_time_taken == min(current_time_taken, half_time_taken, double_time_taken):
            high_search_value = (calibrated_mutation_rate + high_search_value) // 2
            low_search_value = (calibrated_mutation_rate + low_search_value) // 2
    ...
    if(high_search_value == low_search_value):
        calibrated_mutation_rate = low_search_value
    print(f"Optimal mutation_rate: {calibrated_mutation_rate}")
    return calibrated_mutation_rate

def mutation_rate_tuner_2(board_size_n, boards_per_generation,pop_init_alg, mutation_rate, max_generations, stall_limit,stop_tolerence,itererations, ignore_failed_attempts, n_queen_solver):
    ...
    def parameter_avg_time_n_queens_solver(new_mutation_rate):
        return avg_time_n_queens_solver(board_size_n, boards_per_generation,pop_init_alg, new_mutation_rate, max_generations, stall_limit, itererations, ignore_failed_attempts, True , n_queen_solver)
